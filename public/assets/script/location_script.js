$(document).ready(function (e) {
    ...
    let districtContainerID = '#districts';
    let districtOptions = $(districtContainerID);
    ...
    provinceOptions.on('change', function (e) {
        ...
        fetch_cities(selectedProvince, function (response) {
            ...
            cityOptions.on('change', function (e) {
                ...
                fetch_districts(selectedCity, function (response) {
                    districtOptions.html("");
                    if (response.length > 0) {
                        response.forEach(function (v) {
                            districtOptions.append("<option value='" + v.districtName + "'>");
                        });
                    }
                });
            });
...
function fetch_districts(city_id, _callBack) {
    let ajaxUrl = applicationDomain + '/repository/location/get-districts/' + city_id;
